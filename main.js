...
let currentStress = 'que no haya estudiado, que me metan en un grupo mejor';
let currentExcuse = 'ya lo haré mañana, primero necesito organizarme';
let currentFear   = 'que me juzguen, que sea tarde para aprender';

function makeInfo(t, stressor, excuse, fear) {
  const tq = `"${t}"`;

  const sText = stressor || 'lo desconocido, la posibilidad de fallar';
  const eText = excuse   || '"ya lo haré mañana, primero necesito organizarme"';
  const fText = fear     || 'el juicio externo, no estar a la altura';
  ...
  return {
    dis: {
      ...
      color: '#3b82f6',
      story: `El sistema recibe la señal: ${tq}. En este momento exacto la tarea no tiene carga emocional — es solo información nueva. Pero el evaluador automático del cerebro ya está activo, y detecta de inmediato: ${sText}.\n\nAntes de que puedas pensarlo conscientemente, stress = 0.8 se asigna. No ha empezado el bucle todavía — pero ya tiene combustible. Este es el único nodo donde actuar no tiene ningún coste.`,
      storyH: `Aparece el pensamiento: "tengo que hacer ${tq}". Por ahora es solo un recordatorio. No es difícil todavía, nadie te juzga todavía.\n\nPero tu cerebro ya está evaluando. Lo que detecta: ${sText}. La idea de hacer algo nuevo que no controlas te pone nervioso. La alarma no ha sonado aún, pero el dedo ya está en el botón. Este es el momento más barato para actuar — antes de que el bucle arranque.`,
      vars: [
        ...
        { name: 'stress', val: '= 0.8', desc: `Se asigna desde el primer segundo porque el sistema detecta: ${sText}. Lo incierto siempre puntúa alto — es un mecanismo de protección que se dispara demasiado pronto.` },
      ...
      varsH: [
        ...
        { name: 'Nivel de alarma', val: 'alto desde el inicio', desc: `Tu cerebro detecta: ${sText}. No ha pasado nada real, pero la alarma ya suena. Es automático — no lo puedes elegir. Solo puedes elegir qué hacer después.` },
    ...
    ame: {
      ...
      color: '#eab308',
      story: `El cerebro no evalúa si ${tq} es objetivamente difícil. Hace una sola pregunta: "¿el estrés que genera supera mi umbral?" Con stress=0.8 y THRESHOLD=0.5, la respuesta es sí.\n\nEl modo defensivo se activa automáticamente — como un interruptor de seguridad que salta antes de que el circuito se funda. El problema es que este interruptor se calibró para amenazas reales, no para academias de inglés ni correos de trabajo. Pero el cerebro no distingue: si el estrés supera el umbral, activa la evasión. Siempre. Sin consultarte.`,
      storyH: `Tu cerebro no evalúa si la tarea es difícil. Evalúa si el estrés que genera supera tu límite. La fuente: ${sText}.\n\nY llegan las preguntas: qué voy a decir, qué me van a preguntar, cómo será, qué pasa si no sé algo. Ninguna tiene respuesta ahora mismo — y esa incertidumbre es exactamente lo que dispara la alarma. No es la tarea. Es no controlar lo que va a pasar. El modo defensivo se activa solo, antes de que lo decidas.`,
      vars: [
        { name: 'stress', val: '= 0.8', desc: `El sistema suma todo lo incierto de ${tq}: ${sText}. No distingue entre amenaza real y percibida — dispara la alarma por si acaso.` },
      ...
      varsH: [
        { name: 'Estrés percibido', val: 'muy alto', desc: `Tu cerebro suma todo lo incierto: ${sText}. No distingue entre peligro real e imaginado — activa la alarma por si acaso. Por eso es tan difícil de controlar conscientemente.` },
    ...
    eva: {
      ...
      color: '#ef4444',
      story: `avoid() genera la justificación: "${eText}". No es mentira — probablemente tiene algo de verdad. Pero eso es lo que la hace irrebatible. avoid() no busca la excusa perfecta; busca la excusa mínima que pase la validación interna y corte el estrés de ahora.\n\nEl sistema no piensa en mañana. Solo ejecuta la acción más eficiente para eliminar la tensión presente. Desde esa lógica, funciona perfectamente. El problema es que sus consecuencias viven en el futuro.`,
      storyH: `"${eText}." La excusa llega sin esfuerzo, sin deliberación. No es que seas débil — tu cerebro es extremadamente eficiente generando razones para no hacer lo que da miedo.\n\nSiempre encuentra una que no puedes rebatir: tiene algo de verdad, suena razonable, y promete que mañana será mejor momento. "Ya iré mañana." "Hoy tengo que hacer otras cosas." "Primero necesito organizarme." El cerebro no piensa en mañana — solo quiere eliminar la tensión de ahora. Y lo consigue.`,
      vars: [
        { name: 'avoid(task)', val: '→ excusa_válida', desc: `Output: "${eText}". La función siempre devuelve algo creíble. No falla nunca. Pasa la validación interna y corta el estrés inmediatamente.` },
      ...
      varsH: [
        { name: 'La excusa', val: `"${eText}"`, desc: 'Tiene algo de verdad. Eso la hace irrebatible. Pero su función real no es ayudarte — es eliminar el estrés de este momento.' },
    ...
    cul: {
      ...
      color: '#eab308',
      story: `El alivio se disuelve. El sistema genera el diagnóstico: "llevas días con ${tq} pendiente." Cada iteración suma 0.1 a guilt.\n\nEl problema no es que la culpa sea injusta — es que no sirve. No motiva a actuar. La investigación de Kristin Neff (2012) lo confirma: la culpa eleva el estrés percibido y baja el umbral de tolerancia. El siguiente ciclo de evasión es más probable, no menos. La culpa no es gasolina — es arena en el motor.`,
    ...
    reb: {
      ...
      color: '#ef4444',
      story: `La tarea —${tq}— no ha cambiado. Pero cada iteración, el modelo mental de su dificultad se multiplica por 1.2. Después de 7 días: 1.0 × 1.2⁷ = 3.58×. La misma tarea se percibe tres veces y media más difícil.\n\nEl sistema construye narrativas para justificar esa percepción: ${fText}. Ninguna es un hecho — son predicciones bajo estrés. El rebote devuelve el control a La Amenaza con todo empeorado: más dificultad percibida, más culpa, menos tolerancia.`,
      ...
      vars: [
        ...
        { name: 'narrativa generada', val: '"ahora es peor"', desc: `El sistema genera justificaciones: ${fText}. Son predicciones, no hechos. El cerebro las trata con la misma urgencia que un peligro real.` },
    ...
    p1: {
      ...
      color: '#22c55e',
      story: `La Amenaza existe porque ${tq} genera stress=0.8, que supera THRESHOLD=0.5. Pero ese 0.8 no viene de la tarea en sí — viene de todo lo que el sistema proyecta alrededor: ${sText}. La tarea real tiene un stress mucho menor que la tarea imaginada.\n\nEl fix no es bajar el estrés a voluntad — es separar la primera acción concreta de toda la proyección. Si el input es solo "buscar el teléfono de una academia" en vez de "apuntarme a inglés + hablar con recepción + hacer prueba + empezar clases", la condición "if stress > THRESHOLD" falla. El bloque else — donde vive execute() — puede ejecutarse por primera vez.`,
      storyH: `El estrés no viene de la tarea — viene de todo lo que tu cerebro proyecta alrededor. Cuando piensas en ${tq}, no piensas solo en eso: piensas en ${sText}, en qué te van a decir, cómo será, qué pasará después. Tu cerebro procesa todo eso junto y se bloquea.\n\nEl fix: no encarar todo eso de golpe. Solo la primera acción concreta, la que puedes hacer ahora mismo. No ${tq} completo — solo el primer paso real. "Buscar el teléfono." "Abrir la página web." Eso no da miedo. Tu cerebro lo deja pasar. Y una vez en movimiento, el siguiente paso tampoco da tanto miedo, porque ya no estás parado imaginando — estás haciendo.`,
      vars: [
        { name: 'tarea imaginada', val: 'stress = 0.8', desc: `El sistema no procesa ${tq} como una acción — procesa toda la cadena: ${sText}, qué pasará, cómo será. Todo junto supera el umbral.` },
        { name: 'tarea real (primer paso)', val: 'stress = 0.2', desc: 'Si separas solo la primera acción concreta, el stress baja de 0.8 a 0.2. La condición "if stress > THRESHOLD" falla. La evasión no se activa.' },
        { name: 'implementation intent', val: 'si X → entonces Y', desc: 'Define exactamente cuándo y dónde: "Si son las 18:00, abro el buscador y busco una opción." Específico = ejecutable. Vago = bucle (Gollwitzer, 1999).' }
      ],
      varsH: [
        { name: 'Lo que bloquea', val: 'la tarea imaginada, no la real', desc: `Tu cerebro no procesa ${tq} como una acción — procesa toda la cadena de cosas que podrían pasar: ${sText}. Todo junto es demasiado. La acción sola, no.` },
        { name: 'El primer paso concreto', val: 'solo uno, el de ahora mismo', desc: `No ${tq} entero. Solo la primera acción que puedes hacer sin que te dé miedo. "Buscar el teléfono." "Abrir la web." Eso no activa la alarma. Y una vez en movimiento, el siguiente paso cuesta menos.` },
        { name: 'Cuándo y dónde', val: 'decide ahora los dos', desc: '"Si son las X y estoy en Y, haré Z." Ponerle hora y lugar concretos multiplica por 3 la probabilidad de hacerlo (Gollwitzer, 1999). La vaguedad es cómplice del bucle.' }
      ],
    ...
    p3: {
      ...
      color: '#22c55e',
      story: `La culpa acumulada es memoria sin liberar: ocupa recursos, eleva el stress y erosiona el THRESHOLD. El bucle se hace más fácil de activar.\n\nKristin Neff (2012): la autocompasión reduce la procrastinación más que la autocrítica — no porque no importe la tarea, sino porque sin culpa el próximo intento empieza desde una posición más favorable.`,
      storyH: `La culpa siente que debería motivarte. La investigación dice lo contrario: cuanta más culpa, más difícil es empezar la siguiente vez.\n\n"${fText}" — eso no es lo que va a pasar. Es lo que tu cerebro predice bajo presión. Soltar esa historia no significa que no te importa. Significa que vas a intentarlo sin el peso extra de los días anteriores. Tratarte bien ante el fallo mejora el rendimiento futuro (Neff, 2012). No es blandura — es eficiencia del sistema.`,
      vars: [
      ...
      varsH: [
        { name: 'Lo que sueltas', val: 'el peso de los días anteriores', desc: `"${fText}" — una predicción, no un hecho. No tiene que definir el próximo intento.` },
...
let INFO = makeInfo(currentTask, currentStress, currentExcuse, currentFear);
...
function applyTask() {
  ...
  currentFear   = fear;
  INFO = makeInfo(t, stress, excuse, fear);
